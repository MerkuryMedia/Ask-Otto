...
const saveDayButton = document.getElementById("btnSaveDay");
...
function setupEventListeners() {
  ...
  if (saveDayButton) {
    saveDayButton.addEventListener("click", handleSaveDaySelections);
...
function renderDayDetail(targetIso) {
  ...
  field1Select.dataset.iso = targetIso;
  field2Select.dataset.iso = targetIso;
  field3Select.dataset.iso = targetIso;
  if (saveDayButton) {
    saveDayButton.dataset.iso = targetIso;
  }
...
function updateDayDetailSubmitState() {
  ...
  const isComplete = plan && isValidTriplet(plan.field1, plan.field2, plan.field3);
  if (isComplete) {
    info.textContent = "Selections ready. Tap Save & Generate.";
  } else {
    info.textContent = "Select all three fields.";
  }
  if (saveDayButton) {
    saveDayButton.disabled = !isComplete;
  }
}

function handleSaveDaySelections() {
  const iso = field1Select.dataset.iso;
  const weekIndex = Number(field1Select.dataset.weekIndex);
  if (!iso || Number.isNaN(weekIndex)) {
    return;
  }
  const plan = state.weeklyPlan[weekIndex];
  if (!plan || !isValidTriplet(plan.field1, plan.field2, plan.field3)) {
  ...
  if (iso === todayInfo.iso) {
    delete state.challengeByDate[iso];
    if (state.resultsByDate[iso]) {
      delete state.resultsByDate[iso];
      recomputeEmaProgress();
      renderProgress();
    }
    state.lastValueMovedToday = null;
